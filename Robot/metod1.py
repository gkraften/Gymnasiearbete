...
try:
    ...
    while True:
        ...
        if (left <= 30 and right <= 30) and (last_left <= 40 and last_right <= 40):
            ...
            distance.start_measuring(count)
            motors.forward()
        elif right <= 30 and last_right <= 40:
            distance.stop_measuring()
            motors.stop()
            robot.turn_to(compass.getHeading() + math.pi/2)
            last = compass.getHeading()
            distance.start_measuring(count)
            motors.forward()
        elif middle <= 30 and last_right <= 40:
            distance.stop_measuring()
            motors.stop()
            robot.turn_to(compass.getHeading() + math.pi/2)
            last = compass.getHeading()
